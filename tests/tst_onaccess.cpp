...
void TestOnAccess::parseLine_data()
{
    ...
    // Lignes relevées sur clamonacc 1.5 (--fdpass).
    QTest::newRow("détection") << "/home/u/facture.pdf.exe: Win.Test.EICAR_HDB-1 FOUND" << true << true
                               << "/home/u/facture.pdf.exe" << "Win.Test.EICAR_HDB-1" << QString();
    QTest::newRow("« : » dans le nom")
        << "/tmp/w/Mon dossier/facture: mars.pdf.exe: LinuxDefender.Test.Signature.UNOFFICIAL FOUND" << true << true
        << "/tmp/w/Mon dossier/facture: mars.pdf.exe" << "LinuxDefender.Test.Signature.UNOFFICIAL" << QString();
    QTest::newRow("date en préfixe") << "Wed Sep 24 10:12:13 2026 -> /home/u/a.exe: Eicar FOUND" << true << true
                                     << "/home/u/a.exe" << "Eicar" << QString();
    QTest::newRow("erreur fanotify") << "ERROR: Clamonacc: fanotify_init failed: Operation not permitted" << true
                                     << false << QString() << QString()
                                     << "Clamonacc: fanotify_init failed: Operation not permitted";
    QTest::newRow("surveillance") << "ClamInotif: watching '/home' (and all sub-directories)" << false << false
                                  << QString() << QString() << QString();
    QTest::newRow("séparateur") << "--------------------------------------" << false << false << QString()
                                << QString() << QString();
    QTest::newRow("FOUND sans chemin") << "FOUND" << false << false << QString() << QString() << QString();
}
...
void TestOnAccess::parseLine()
{
    ...
    QCOMPARE(parsed.has_value(), recognized);
    if (!parsed)
        return;
    QCOMPARE(parsed->type == OnAccessLogLine::Type::Detection, detection);
...
void TestOnAccess::explainError_data()
{
    ...
    QTest::newRow("inotify") << "ClamInotif: could not watch path, No space left on device (inotify)"
                             << "max_user_watches";
...
void TestOnAccess::stateFromSystemd_data()
{
    ...
    QTest::newRow("service absent") << "not-found" << "inactive" << OnAccessController::State::ServiceMissing;
    QTest::newRow("arrêté") << "loaded" << "inactive" << OnAccessController::State::Inactive;
    QTest::newRow("actif") << "loaded" << "active" << OnAccessController::State::Active;
    QTest::newRow("en échec") << "loaded" << "failed" << OnAccessController::State::Failed;
}
...
void TestOnAccess::stateFromSystemd()
{
    ...
    QSignalSpy changed(onAccess.get(), &OnAccessController::stateChanged);
    onAccess->refresh();
    QTRY_COMPARE_WITH_TIMEOUT(onAccess->state(), state, 3000);
    QVERIFY(!onAccess->message().isEmpty());
}
